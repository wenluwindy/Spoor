...
const FIELD =
  'w-full bg-[#FAF9F6] border border-[#E6E4DF] rounded-lg text-xs outline-none focus:border-[#C2410C] transition-all';
...
export function ImageGenNode({
  ...
  return (
    <div className="w-full h-full bg-white shadow-lg border-2 border-[#E6E4DF] flex flex-col rounded-xl overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#F4F1ED] bg-[#FAF9F6]">
        <Sparkles className="w-3.5 h-3.5 text-[#C2410C] shrink-0" />
        <span className="text-[10px] font-sans font-bold uppercase tracking-wider text-[#8c8a84] shrink-0">
          {t('imagegen.title')}
        </span>
        <select
          className="ml-auto min-w-0 max-w-[55%] text-[11px] bg-transparent border border-[#E6E4DF] rounded px-1.5 py-0.5 outline-none focus:border-[#C2410C]"
      ...
      {/* ── 预览区：空 / 生成中 / 完成 / 错误 ── */}
      <div className="flex-1 min-h-[160px] flex items-center justify-center bg-[#F4F1ED] relative">
        {isGenerating ? (
          <div className="flex flex-col items-center gap-2 text-[#8c8a84]">
          ...
          <div className="flex flex-col items-center gap-1.5 px-4 py-3 text-center">
            ...
            {node.imageGenErrorDetail && (
              <details className="w-full">
                <summary className="text-[10px] text-[#8c8a84] cursor-pointer">
                  {t('imagegen.error_detail')}
                </summary>
                <p className="text-[10px] text-[#8c8a84] break-all text-left mt-1">
                  {node.imageGenErrorDetail}
                </p>
              </details>
            )}
          ...
          <div className="px-6 py-8 text-center text-[11px] text-[#8c8a84] leading-relaxed border-2 border-dashed border-[#d1cfca] m-3 rounded-lg">
      ...
      {results.length > 0 && !isGenerating && (
        <div className="flex items-center justify-center gap-2 px-3 py-1.5 border-t border-[#F4F1ED] text-[11px] text-[#5a5a54]">
          <Tooltip label={t('imagegen.prev_result')}>
            <button
              type="button"
              disabled={activeIndex <= 0}
              onClick={() => onSetActiveIndex(activeIndex - 1)}
              className="p-0.5 disabled:opacity-30 hover:text-[#C2410C] transition-colors"
            >
          ...
          <Tooltip label={t('imagegen.delete_result')}>
            <button
              ...
              className="p-0.5 ml-1 hover:text-[#C2410C] transition-colors"
            >
      ...
      {neighborRefs.length > 0 && (
        <div className="flex items-center gap-1.5 px-3 py-1.5 border-t border-[#F4F1ED] flex-wrap">
          <span className="text-[10px] text-[#8c8a84] shrink-0">{t('imagegen.references')}</span>
              ...
              >
                <button
                  ...
                  className={`w-7 h-7 rounded border overflow-hidden shrink-0 transition-opacity ${
                    excluded ? 'opacity-30 border-[#E6E4DF]' : 'border-[#C2410C]/40'
                  }`}
                >
                  <img
                    alt=""
          ...
          {inputs.ignoredRefCount > 0 && (
            <span className="text-[10px] text-[#C2410C]">
      ...
      {/* ── 提示词与生成 ── */}
      <div className="px-3 py-2 border-t border-[#F4F1ED] space-y-2">
        {inputs.upstreamText && (
          <label className="flex items-center gap-1.5 text-[10px] text-[#8c8a84] cursor-pointer">
            <input
              type="checkbox"
        ...
        <div className="flex items-center gap-2">
          {target?.model.sizeOptions && target.model.sizeOptions.length > 0 && (
            <select
              className="text-[11px] bg-[#FAF9F6] border border-[#E6E4DF] rounded px-1.5 py-1 outline-none focus:border-[#C2410C]"
              aria-label={t('imagegen.size')}
              value={node.imageGenParams?.size ?? target.model.defaultParams?.size ?? ''}
              onChange={(e) =>
                onPatch({ imageGenParams: { ...node.imageGenParams, size: e.target.value } })
              }
            >
              {target.model.sizeOptions.map((size) => (
          ...
          <button
            ...
            className="ml-auto px-4 py-1.5 bg-[#C2410C] text-white rounded-lg text-xs font-bold hover:bg-[#9A3412] transition-colors disabled:opacity-40 shrink-0 flex items-center gap-1.5"
          >
            {isGenerating ? <Loader2 className="w-3 h-3 animate-spin" /> : null}
            {t('imagegen.generate')}
          </button>
        </div>
        {!target && <p className="text-[10px] text-[#C2410C]">{t('imagegen.need_model')}</p>}
        {target && !prompt && <p className="text-[10px] text-[#8c8a84]">{t('imagegen.need_prompt')}</p>}
        {inputs.hasImageGenNeighbor && (
          <p className="text-[10px] text-[#8c8a84]">{t('imagegen.chain_hint')}</p>
        )}
      </div>
    </div>
  );
